...
export const customMessageHandlers =
  (
  ...
  async (message: IMessage) => {
    if (!client.openaiApiKey) {
      ...
      if (inputApiKey) {
        ...
        vscode.window.showErrorMessage(
          "OpenAI API Key is required for this step."
        );
        // Send a response back to the webview
        panel.webview.postMessage({
          command: MessageCommands.LEARN,
          result: {},
        });
    ...
    switch (command) {
      case MessageCommands.GET_GLOBAL_STATE:
        panel.webview.postMessage({
          ...
          payload: JSON.stringify({
            ...
            autoCleanUp: context.globalState.get("autoCleanUp"),
          }),
        } as MessageHandlerData<string>);
        break;

      case MessageCommands.SCAN:
        const filePaths = await scanWorkspace(
          context.globalState.get("matchPattern"),
          (context.globalState.get("excludePatterns") as string)?.split("\n")
        );
      ...
      case MessageCommands.LEARN:
        ...
        if (gifDiff) {
          const answer =
            await client.ask(`Please sum up what did you learn in bulletin points and less than 100 words.
          Git diff result:\n${gifDiff}`);
          ...
          vscode.window.showWarningMessage(
            "No staged files found. Please stage your changes and try again."
          );
      ...
      case MessageCommands.GENERATE:
        ...
        const codemod = await client.ask(
          context.globalState.get("promptGenerate", defaultGeneratePrompt)
        );

        panel.webview.postMessage({
          command,
          requestId,
          payload: stripCodeBlockAnnotations(
            codemod || "No codemod script generated."
          ),
        } as MessageHandlerData<string>);
        ...
        codemod &&
          context.globalState.update(
            "codemodScript",
            stripCodeBlockAnnotations(codemod)
          );
      ...
      case MessageCommands.STORE_DATA:
        ...
        payload.matchPattern &&
          context.globalState.update("matchPattern", payload.matchPattern);
        payload.excludePatterns &&
          context.globalState.update(
            "excludePatterns",
            payload.excludePatterns
          );
        payload.promptSystem &&
          context.globalState.update("promptSystem", payload.promptSystem);
        payload.promptGenerate &&
          context.globalState.update("promptGenerate", payload.promptGenerate);
        payload.codemodScript &&
          context.globalState.update("codemodScript", payload.codemodScript);
        payload.autoCleanUp !== undefined &&
          context.globalState.update("autoCleanUp", payload.autoCleanUp);
      ...
      case MessageCommands.APPLY_REQUEST:
        vscode.window.showInformationMessage(
          "Applying codemod to selected files..."
        );

        const codemodScript = context.globalState.get(
          "codemodScript"
        ) as string;
        ...
        if (codemodScript) {
          try {
            await applyCodemod(
              ...
              context.globalState.get("autoCleanUp", true)
            );
          } catch (error) {
            vscode.window.showErrorMessage(
              `Applying error: ${
            ...
            panel.webview.postMessage({
              command: MessageCommands.APPLY_RESULT,
              result: {},
            });
          }
